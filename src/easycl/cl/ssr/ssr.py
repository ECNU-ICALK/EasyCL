...
class SSR:
    ...
    def get_few_shot_examples(self, raw_data: List[Dict[str, Any]], num_shots: Optional[int] = None):
        ...
        if len(raw_data) < num_shots:
            ...
            effective_num_shots = len(raw_data)
        else:
    ...
    def generate_pseudo_samples(self, num_samples: Optional[int] = None):
        ...
        debugprint(f"generate_pseudo_samples 完成，返回 {len(pseudo_samples)} 个伪样本")
    ...
    def refine_pseudo_samples(self, pseudo_samples):
        ...
        logger.info(f"已优化 {len(refined_samples)} 个伪样本")
    ...
    def select_diverse_samples(self, pseudo_samples, max_samples=None):
        ...
        使用聚类方法选择多样化的伪样本
        """
        debugprint(f"select_diverse_samples 开始，待选样本数: {len(pseudo_samples)}, 最大样本数: {max_samples or self.pseudo_sample_memory}")
        ...
        if self.sentence_encoder is None:
            ...
            debugprint("select_diverse_samples 完成 (随机选择)")
        ...
        try:
            ...
            selected_samples = random.sample(pseudo_samples, max_samples)
            debugprint(f"随机选择了 {len(selected_samples)} 个伪样本")
            debugprint("select_diverse_samples 完成 (SimCSE 失败后随机选择)")
            return selected_samples
        ...
        debugprint(f"select_diverse_samples 完成 (聚类选择)，返回 {len(selected_samples)} 个样本")
        return selected_samples
    ...
    def save_pseudo_samples(self, samples, task_id, prev_task_id=None):
        ...
        保存伪样本到指定目录
        """
        debugprint(f"save_pseudo_samples 开始，当前任务ID: {task_id}, 上一任务ID: {prev_task_id}, 待保存样本数: {len(samples)}")
        # 构建保存路径
        ...
        debugprint(f"save_pseudo_samples 完成，返回任务目录: {task_dir}")
        return task_dir
