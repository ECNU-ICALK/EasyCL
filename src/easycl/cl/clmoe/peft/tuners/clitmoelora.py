...
if is_bnb_available():
    ...
    # Conditionally import Linear8bitLt and Linear4bit if bnb is available
...
class CLMoEMOELoraLinear(nn.Linear, CLMoEMOELoraLayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            task = "unknown_task" # Default task name
            if self.output_dir:
                # --- New path calculation for reading ---
                original_output_dir = self.output_dir
                parent_dir = os.path.dirname(original_output_dir)
                base_name = os.path.basename(original_output_dir)
                # Handle edge case matching the writing logic
                if not parent_dir:
                    parent_dir = "."
                ...
                try:
                    ...
                    # Keep warning, but update the expected path message
                    warnings.warn(f"Task file not found at the expected new location: {task_file_path}. Using default task name '{task}'.")
                except Exception as e:
                    warnings.warn(f"Error reading task file at {task_file_path}: {e}. Using default task name '{task}'.")
            else:
