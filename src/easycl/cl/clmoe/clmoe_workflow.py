...
def run_statistics_calculation(output_dir: str, task_id: str, top_k_experts: int):
    ...
    if is_main_process():
        if not os.path.exists(value_counts_file):
            # 主进程检测到文件不存在，记录错误
            logger.error(f"Statistics input file not found: {value_counts_file}. Cannot proceed.")
            file_exists = False
        else:
            file_exists = True
    else:
    ...
    # 在分布式环境中，广播文件存在状态
    if is_distributed():
        file_exists_tensor = torch.tensor([1 if file_exists else 0], device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    try:
        ...
        if is_main_process():
            ...
            with open(value_counts_file, "r", encoding="utf-8") as txt_file:
                for line in txt_file:
                    try:
                        ...
                        logger.warning(f"Skipping malformed line in {value_counts_file}: {line.strip()}")
                        continue
            ...
            logger.info(f"Successfully calculated statistics and saved top {top_k_experts} indices to {index_output_file}")
            logger.info(f"Statistics result: {sorted_result}")

        # 在分布式环境中，确保所有进程都等待主进程完成计算
        if is_distributed():
            torch.distributed.barrier()
        ...
        # 只在主进程中记录错误
        if is_main_process():
            logger.error(f"Error during statistics calculation for {task_id}: {e}")
        # 在分布式环境中，确保所有进程都抛出相同的异常
        if is_distributed():
            torch.distributed.barrier()
...
def run_parameter_alignment(output_dir: str, current_task_id: str, prev_task_id: str, prev_task_model_path: str):
    ...
    if is_main_process():
        ...
        if missing_files:
            # 主进程检测到文件不存在，记录错误
            logger.error(f"Missing required files for parameter alignment: {', '.join(missing_files)}. Cannot proceed.")
            files_exist = False
        else:
            files_exist = True
    else:
    ...
    # 在分布式环境中，广播文件存在状态
    if is_distributed():
        files_exist_tensor = torch.tensor([1 if files_exist else 0], device="cuda" if torch.cuda.is_available() else "cpu")
        torch.distributed.broadcast(files_exist_tensor, src=0)
        files_exist = bool(files_exist_tensor.item())

    # 如果文件不存在，所有进程都抛出异常
    if not files_exist:
        raise FileNotFoundError(f"Missing required files for parameter alignment. Cannot proceed.")
    ...
    try:
        ...
        if is_main_process():
            ...
            logger.info(f"Exclusive expert indices (in {prev_task_id} only): {exclusive_indices}")
            ...
            logger.info(f"Parameter alignment summary: Merged {merged_count} tensors, Skipped/Kept {skipped_count} tensors.")
            if problematic_keys:
                logger.warning(f"Problematic keys during merge (check patterns): {problematic_keys[:10]}...")
            ...
            logger.info(f"Copying config/tokenizer files from {not_aligned_dir} to {output_dir}")
            ...
            logger.info(f"Saved aligned adapter weights to {final_adapter_path}")
            # --- End Save ---

            logger.info("Parameter alignment completed successfully.")
        ...
        # 只在主进程中记录错误
        if is_main_process():
            logger.error(f"Error during parameter alignment: {e}")
        # 在分布式环境中，确保所有进程都抛出相同的异常
        if is_distributed():
            torch.distributed.barrier()
