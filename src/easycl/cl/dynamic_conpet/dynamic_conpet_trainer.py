...
class DynamicConPetTrainer(CustomSeq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False,num_items_in_batch=None):
        ...
        debugprint(f"分类损失权重: {self.classification_loss_weight}")
        debugprint(f"计算出的分类损失: {classification_loss.item()}")
        
        # Combine losses
        total_loss = original_loss + self.classification_loss_weight * classification_loss
        debugprint(f"总损失 (原始损失 + 分类损失 * 权重): {total_loss.item()}")
        ...
        debugprint(f"记录日志数据: {log_data}")
        self.log(log_data)
        
        debugprint("compute_loss 方法结束")
        if return_outputs:
    ...
    def save_model(self, *args, **kwargs):
        ...
        """
        debugprint("进入 save_model 方法")
        # Call original save method
        output = super().save_model(*args, **kwargs)
        debugprint(f"原始 save_model 返回: {output}")
        ...
        if self.dataset_classifier is not None and self.args.should_save:
            ...
            debugprint(f"准备保存数据集分类器到: {classifier_save_path}")
            save_classifier(self.dataset_classifier, classifier_save_path, self.dataset_names)
        else:
            debugprint("不保存数据集分类器 (可能未提供或 should_save 为 False)")
        
        debugprint("save_model 方法结束")
        return output
    ...
    def get_batch_dataset_labels(self, batch_indices: List[int]) -> torch.Tensor:
        ...
        """
        debugprint(f"进入 get_batch_dataset_labels, batch_indices: {batch_indices}")
        labels = []
        for idx in batch_indices:
            ...
            debugprint(f"样本索引 {idx}, 查找数据集，数据集索引映射: {sorted_map_items}")
            for (start_idx, end_idx), ds_idx in sorted_map_items:
                if start_idx <= idx < end_idx:
                    dataset_idx = ds_idx
                    found_dataset = True
                    debugprint(f"样本索引 {idx} 属于数据集索引 {ds_idx} (范围 [{start_idx}, {end_idx}))")
                    break
            if not found_dataset:
                 ...
                 debugprint(f"警告: 样本索引 {idx} 未在 dataset_indices_map 中找到，默认使用索引 {default_idx}")
                 dataset_idx = default_idx

            labels.append(dataset_idx)

        result_tensor = torch.tensor(labels, device=self.args.device)
        debugprint(f"生成的 dataset_labels 张量: {result_tensor}")
    ...
    def _prepare_inputs(self, inputs):
        ...
        if self.dataset_classifier is not None and self.dataset_indices_map:
            ...
            if "index" in prepared_inputs: # Check in prepared_inputs which is the result of super call
                ...
                debugprint(f"提取到的批次索引: {batch_indices}")
                ...
                debugprint("已添加 dataset_labels 到准备好的输入中")
            else:
                debugprint("警告: 在准备好的输入中未找到 'index'，无法添加数据集标签")
        else:
            debugprint("不需要添加数据集标签 (无分类器或无索引映射)")
        
        debugprint("_prepare_inputs 方法结束")
