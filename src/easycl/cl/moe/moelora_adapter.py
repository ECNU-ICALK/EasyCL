...
import torch
# Use local peft library instead of original peft library
from .peft import LoraConfig, CoINMOELoraConfig, PeftModel, TaskType, get_peft_model
...
from llamafactory.model.model_utils.visual import get_forbidden_modules, patch_target_modules
...
def _setup_moelora_tuning(
    ...
    """
    Setup model with MOE-LoRA
    """
    if is_trainable:
        if hasattr(cl_finetuning_args, "expert_num") and cl_finetuning_args.expert_num > 1:
            logger.info_rank0(f"Fine-tuning method: MOE-LoRA with {cl_finetuning_args.expert_num} experts")
        else:
            logger.info_rank0(f"Fine-tuning method: {('DoRA' if finetuning_args.use_dora else 'LoRA')}")
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        # Determine which configuration to use based on expert_num
        if hasattr(cl_finetuning_args, "expert_num") and cl_finetuning_args.expert_num > 1:
            if hasattr(cl_finetuning_args, "task_embedding_dim") and cl_finetuning_args.task_embedding_dim > 0:
                task_embedding_dim = cl_finetuning_args.task_embedding_dim
            else:
                task_embedding_dim = 64  # Default value
                
            logger.info_rank0(f"Using MOE-LoRA with {cl_finetuning_args.expert_num} experts and task embedding dim {task_embedding_dim}")
            
            # Use CoINMOELoraConfig instead of LoraConfig
            ...
            # Use original LoRA configuration
            if model_args.use_unsloth:
        ...
        # If not using unsloth, apply get_peft_model
...
def init_moelora_adapter(
    ...
    r"""
    Initializes the adapters with MOE-LoRA.

    Support full-parameter, freeze and MOE-LoRA training.
